...
class Attention(tf.keras.layers.Layer):
    def __init__(self, num_head, size_per_head):
        ...
        initializer = tf.initializers.glorot_uniform(seed=196)
        # self.weight_q = tf.Variable(initializer(shape=(num_head, size_per_head, size_per_head), dtype=tf.float32))
        # self.weight_k = tf.Variable(initializer(shape=(num_head, size_per_head, size_per_head), dtype=tf.float32))
        # self.weight_v = tf.Variable(initializer(shape=(num_head, size_per_head, size_per_head), dtype=tf.float32))
        # self.weight_o = tf.Variable(initializer(shape=(num_head, size_per_head, size_per_head), dtype=tf.float32))
        self.weight_q = tf.Variable(initial_value=np_q,dtype=tf.float32)
        self.weight_k = tf.Variable(initial_value=np_k,dtype=tf.float32)
        self.weight_v = tf.Variable(initial_value=np_v,dtype=tf.float32)
        self.weight_o = tf.Variable(initial_value=np_o,dtype=tf.float32)
        tf.keras.initializers.glorot_uniform(self.weight_q)
        tf.keras.initializers.glorot_uniform(self.weight_k)
        tf.keras.initializers.glorot_uniform(self.weight_v)
        tf.keras.initializers.glorot_uniform(self.weight_o)

        print(self.weight_q,self.weight_k,self.weight_v,self.weight_o)
...
def test_all():
    ...
    test_res_accuracy(model, x,k,v)
    
def test_res_accuracy(model,x,k,v):
        print("----test-res-accuracy----")
        #warm_up
        k,v,x = model(x,k,v,training=False)
        print(x.numpy().mean())
